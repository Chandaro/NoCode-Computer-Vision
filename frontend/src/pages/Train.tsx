...
export default function Train() {
  ...
  // Pre-flight resource estimate — refetch (debounced) when the cost knobs change
  useEffect(() => {
    const t = setTimeout(() => {
      api.get(`/projects/${projectId}/training/estimate`, {
        params: { model_base: modelBase, imgsz, batch, freeze },
      }).then(r => setEstimate(r.data)).catch(() => setEstimate(null))
    }, 300)
    return () => clearTimeout(t)
  }, [projectId, modelBase, imgsz, batch, freeze])
